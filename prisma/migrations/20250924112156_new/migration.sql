...
CREATE TABLE `User` (
    ...
    `lastSessionInvalidation` DATETIME(3) NULL,

    UNIQUE INDEX `User_email_key`(`email`),
    UNIQUE INDEX `User_googleId_key`(`googleId`),
    UNIQUE INDEX `User_otpCode_key`(`otpCode`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `Product` (
    ...
    `availableColors` VARCHAR(191) NULL,
...
CREATE TABLE `Order` (
    ...
    UNIQUE INDEX `Order_transactionNumber_key`(`transactionNumber`),
    INDEX `Order_userId_idx`(`userId`),
    INDEX `Order_shippingAddressId_idx`(`shippingAddressId`),
    INDEX `Order_paymentMethodId_idx`(`paymentMethodId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `OrderItem` (
    ...
    INDEX `OrderItem_orderId_productId_idx`(`orderId`, `productId`),
...
CREATE TABLE `DeliveryFee` (
    ...
    UNIQUE INDEX `DeliveryFee_country_city_key`(`country`, `city`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- CreateTable
CREATE TABLE `_ProductOnWishlist` (
    `A` VARCHAR(191) NOT NULL,
    `B` VARCHAR(191) NOT NULL,

    UNIQUE INDEX `_ProductOnWishlist_AB_unique`(`A`, `B`),
    INDEX `_ProductOnWishlist_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `_ProductOnWishlist` ADD CONSTRAINT `_ProductOnWishlist_A_fkey` FOREIGN KEY (`A`) REFERENCES `Product`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_ProductOnWishlist` ADD CONSTRAINT `_ProductOnWishlist_B_fkey` FOREIGN KEY (`B`) REFERENCES `Wishlist`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
